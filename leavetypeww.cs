...
namespace GeneXus.Programs {
   public class leavetypeww : GXDataArea
   {
      ...
      protected void E192O2( )
      {
         ...
         AV64QueryIntent = StringUtil.Trim( AV27Session.Get(AV66Pgmname+"QueryIntent"));
         AssignAttri("", false, "AV64QueryIntent", AV64QueryIntent);
         GxWebStd.gx_hidden_field( context, "gxhash_vQUERYINTENT", GetSecureSignedToken( "", StringUtil.RTrim( context.localUtil.Format( AV64QueryIntent, "")), context));
         if ( ! String.IsNullOrEmpty(StringUtil.RTrim( AV64QueryIntent)) )
         {
            AV27Session.Remove(AV66Pgmname+"QueryIntent");
            if ( StringUtil.StrCmp(AV64QueryIntent, "5") == 0 )
            {
               if ( new WorkWithPlus.workwithplus_commongam.secgamisauthbyfunctionalitykey(context).executeUdp(  "leavetype_Insert") )
               {
      ...
      private void E202O2( )
      {
         ...
         if ( AV8RedirectToFirstDetail )
         {
            if ( StringUtil.StrCmp(AV64QueryIntent, "2") == 0 )
            {
               ...
               if ( new WorkWithPlus.workwithplus_commongam.secgamisauthbyfunctionalitykey(context).executeUdp(  "leavetypeview_Execute") )
               {
            ...
            {
               if ( new WorkWithPlus.workwithplus_commongam.secgamisauthbyfunctionalitykey(context).executeUdp(  "leavetype_Update") )
               {
            ...
            {
               if ( new WorkWithPlus.workwithplus_commongam.secgamisauthbyfunctionalitykey(context).executeUdp(  "leavetype_Delete") )
               {
      ...
      protected void define_styles( )
      {
         ...
         {
            context.AddJavascriptSource(StringUtil.RTrim( ((string)Form.Jscriptsrc.Item(idxLst))), "?20256267543266", true, true);
            idxLst = (int)(idxLst+1);
         }
         if ( ! outputEnabled )
         {
      ...
      protected void include_jscripts( )
      {
         ...
         context.AddJavascriptSource("leavetypeww.js", "?20256267543268", false, true);
         context.AddJavascriptSource("DVelop/Bootstrap/Shared/DVelopBootstrap.js", "", false, true);
         context.AddJavascriptSource("DVelop/Shared/WorkWithPlusCommon.js", "", false, true);
         context.AddJavascriptSource("DVelop/Bootstrap/DropDownOptions/BootstrapDropDownOptionsRender.js", "", false, true);
         context.AddJavascriptSource("DVelop/Bootstrap/Shared/DVelopBootstrap.js", "", false, true);
